...
| Claude API (Sonnet 4.6 + prompt caching) | 캐싱 70%+ 기대치: $50~$120 / 캐싱 미적용 최악: ~$360 |
| Caddy/DNS/SSL | $0 (Let's Encrypt 자동) |
| **합계 (POC 소규모, 캐싱 적중 가정)** | **$10 ~ $150** |
...
**Google Sheets 경로** (신규):
- 대상 지점: **AMNY** (뉴욕), **DSTX** (reSOUND NY) — 외부 마케팅 대행사 운영, API 접근 불가
...
- 정규화: 4 시트를 `(date, channel_code)` 기준 join → 통합 fact row → `raw_ads.external_ads_raw` 신규 테이블
  - 컬럼: `date`, `branch_id` ('AMNY'/'DSTX'), `channel_code`, `channel_key`, `spend_usd`, `impressions`, `clicks`, `transactions`, `plan_views`, `cr_pct`, `ingestion_ts`, `source_payload JSON`
...
**POC 단계 접근 방식**:
- **스냅샷 범위**: 시트 **전체 이력** 읽기 (AMNY ~3,000 rows + DSTX ~300 rows, BQ 저장량 무시 수준)
- **업데이트 주기**: **일일 자동 cron** — VM 크론이 `viz/scripts/seed-sheets.py --overwrite` 매일 06:00 KST 실행
- 구현 방식: 간단 import 스크립트 + BQ WRITE_TRUNCATE. 본격 sync 어댑터 아님
- **실시간 sync 어댑터**: POC 이후 Sprint (증분 로드 + DQ + 스키마 드리프트 방지). `channels/external_sheets/`로 승격 예정

이유: W1/W2 타임라인 보호하면서도 실사용 2주간 D-1 fresh 데이터 제공. 완전 sync는 POC 이후 안정화.
...
│ - dashboards │ │ - Cube agg │  │ mart.v_blended_funnel_*  │
│ - charts     │ │   cache    │  │ mart.v_*_branch_daily    │
│ - chat_msgs  │ │            │  │ core.fact_*              │
│ - share_tokn │ │            │  │ (governance.* 사용 안함) │
└──────────────┘ └────────────┘  └────────────────────────┘
```
...
- **인증**: `cube.js` 엔트리에서 `checkAuth` 콜백이 JWT(HS256) 검증. app-server와 공유 secret. payload claim: `{ user_id, email, iat, exp (15min) }`. 만료 또는 서명 불일치 시 401
...
- 포트: 5432 내부
- **마이그레이션 정책**: 
  - `postgres/init.sql`: DB/role/extension 부트스트랩만 (docker-compose 최초 1회)
  - Drizzle: 스키마·인덱스 관리. dev = `drizzle-kit push` (자동), prod = `drizzle-kit generate` + `drizzle-kit migrate` (배포 스크립트에서 명시 실행)
- **백업**: 일간 `pg_dump` cron → GCS 업로드 (`gs://{bucket}/mkt-viz-backup/YYYY-MM-DD.sql.gz`). 30일 retention. POC 배포 시 `scripts/deploy.sh`에 포함.
- **로그 retention**: `ai_call_log` — POC 기간 무제한. 프로덕션 전환 시 90일 TTL + `pg_cron` 정리.
...
| Cube API 우회 호출 | 프론트 미노출. app-server가 JWT 서명(HS256) 후 전달. Cube `checkAuth`에서 secret 검증 + 만료 확인 |
| SQL injection | 경로 없음 — Claude → Cube JSON → 파라미터화 SQL |
| CORS | Caddy 리버스 프록시로 단일 origin 노출(`viz.dstrict.com`). Cube API는 내부 네트워크만 노출, 공개 X. Next.js CORS 정책은 same-origin 기본 |
| CSRF | NextAuth 기본 CSRF + 상태 변경 API(`POST/PATCH/DELETE`)에 double-submit cookie 또는 `x-csrf-token` 헤더 검증. `SameSite=Lax` 쿠키 정책 |
| Prompt Injection | 사용자 자연어가 Claude에 전달됨. `@dstrict.com` 내부 사용자만 접근 가능하므로 POC 단계 **인지하되 수용**. 방어 1차: Claude tool_choice로 chart config 스키마만 출력 허용. LLM 응답이 시스템 명령으로 해석될 경로 없음. 프로덕션 전환 시 system prompt 격리·입력 삭제 재평가 |
...
| 일 | 세션 | 작업 |
|---|---|---|
| D1 | S1 | docker-compose (cube+pg+redis) + BQ 연결 + cube schema 3개 (AdsCampaign, Orders, Surveys) + 시드 스크립트 (sales/survey + **AMNY·DSTX 시트 전체 이력 CSV→BQ import**, WRITE_TRUNCATE 전략) |
| D1 | S2 | Cube 스키마 한글 title/description + `dim_branch`에 AMNY/DSTX row 추가 + `governance.external_channel_map` 시드 + Playground 수동 쿼리 검증 |
...
| W2 D5 | S19 | **VM 첫 배포** + smoke + SSL 인증서 발급 확인 + **일일 cron entry 설정** (매일 06:00 KST `seed-sheets.py`) + 시트 freshness 체크 스크립트 |
...
| ~~R22~~ | ~~DSTX 시트 URL 미확보~~ — 해결됨 (2026-04-23 확보) | - | - | 제거 |
| R23 | 일일 cron 실행 실패 감지 (이메일/Slack 알림 미구축) | 중 | 중 | stdout/stderr 로그 파일 + `viz/scripts/check-sheet-freshness.py` 일일 체크. 48h 이상 stale 시 BQ DQ row 생성 + 수동 알림 |
...
- [x] DSTX 시트 4개 GID 확보 — 2026-04-23 완료 (단일 spreadsheet file, 지점별 tab 분리)
- [ ] `core.dim_branch`에 `AMNY`, `DSTX` row 추가 — W1 시드 스크립트에서 병행
- [ ] `governance.external_channel_map` 시드 — W1 S1 cube 스키마 작성과 병행
- [ ] VM 크론 entry 설정 (매일 06:00 KST `seed-sheets.py`) — W2 D5 배포 단계
...
- [ ] 응답 시간 p95 < 5초
- [ ] 실사용자 3명이 2주간 매일 접속
- [ ] 차트 누적 100개+ — 측정: `SELECT COUNT(*) FROM dashboard_charts WHERE created_at >= '실사용 시작일'`, AI/수동/hybrid 모든 source 합산, soft-delete 포함 (deleted_at 기준 구분 없음)
...
- 대행사 시트 **실시간 sync 어댑터** (POC는 일일 cron 스냅샷까지만. 실시간/증분/DQ는 POC 이후 Sprint)
